...
function CouponPreview({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border-2 border-dashed border-emerald-200 bg-gradient-to-br from-emerald-50 to-green-50 px-5 py-5">
      {/* Ticket notch cutouts */}
      <div className="absolute -left-3 top-1/2 h-6 w-6 -translate-y-1/2 rounded-full border-2 border-dashed border-emerald-200 bg-white" />
      <div className="absolute -right-3 top-1/2 h-6 w-6 -translate-y-1/2 rounded-full border-2 border-dashed border-emerald-200 bg-white" />
      ...
      <div className="text-center">
        ...
        <div className="mt-1 text-sm text-slate-500">{displayDesc}</div>
      </div>

      <div className="mx-4 my-4 border-t border-dashed border-emerald-200" />

      <div className="flex items-center justify-between px-1 text-xs">
        <div>
          <div className="text-slate-400">Valid till</div>
          <div className="mt-0.5 font-semibold text-slate-700">{validDate}</div>
        </div>
        <div className="text-right">
          <div className="text-slate-400">Usage limit</div>
          <div className="mt-0.5 font-semibold text-slate-700">{usageText}</div>
        </div>
      </div>
    </div>
  );
...
const inputCls =
  "w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 placeholder-slate-300 outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-100";
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-2xl border border-slate-100 bg-white shadow-sm">
        <div className="flex items-center gap-2.5 border-b border-slate-100 px-5 py-4">
          ...
          <h3 className="text-sm font-semibold text-slate-900">Create New Coupon</h3>
        </div>
        ...
        <form onSubmit={submit} className="space-y-4 px-5 py-4">
          ...
          <div>
            ...
            />
            <p className="mt-1 text-[11px] text-slate-400">Unique code riders will apply</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className={inputCls}
            />
            <p className="mt-1 text-[11px] text-slate-400">Shown in admin panel only</p>
          </div>
          ...
          <p className="-mt-2 text-[11px] text-slate-400">Leave empty for unlimited</p>

          {error && (
            <div className="rounded-xl bg-red-50 px-3 py-2.5 text-xs font-medium text-red-700 ring-1 ring-red-200">
              {error}
            </div>
          )}
          {success && (
            <div className="rounded-xl bg-green-50 px-3 py-2.5 text-xs font-medium text-green-700 ring-1 ring-green-200">
      ...
      <div className="rounded-2xl border border-slate-100 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3">
          ...
          {selectedCoupon ? (
            <button
              onClick={onDeselect}
              className="flex items-center gap-1 rounded-lg px-2 py-0.5 text-[11px] text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors"
            >
              <X className="h-3 w-3" />
              Clear
            </button>
          ) : (
            <span className="text-[11px] text-slate-400">See how it will look</span>
          )}
        </div>
