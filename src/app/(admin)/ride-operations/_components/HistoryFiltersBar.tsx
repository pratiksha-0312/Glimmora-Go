...
import { Search, SlidersHorizontal, MapPin, ChevronDown, Check, RotateCcw, CalendarDays } from "lucide-react";
...
export function HistoryFiltersBar({
  ...
  function apply(overrides?: { channel?: string; status?: string; dateFrom?: string; dateTo?: string }) {
    ...
    router.push(`/ride-operations/history${qs ? `?${qs}` : ""}`);
  }

  function reset() {
    setSearch(""); setDateFrom(""); setDateTo(""); setChannel(""); setStatus("");
    router.push("/ride-operations/history");
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-100 bg-white p-3 shadow-sm dark:border-[#2a2a2a] dark:bg-[#1a1a1a]">
      ...
      <div className="w-40">
        <DateInput
          value={dateFrom}
          onChange={(v) => { setDateFrom(v); apply({ dateFrom: v }); }}
      ...
      <div className="w-40">
        <DateInput
          value={dateTo}
          onChange={(v) => { setDateTo(v); apply({ dateTo: v }); }}
          placeholder="To Date"
        />
      </div>
      ...
      <div className="w-36">
        <FilterSelect
          ...
          value={channel}
          onChange={(v) => { setChannel(v); apply({ channel: v }); }}
      ...
      <div className="w-36">
        <FilterSelect
          ...
          value={status}
          onChange={(v) => { setStatus(v); apply({ status: v }); }}
