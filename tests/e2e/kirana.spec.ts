...
test.describe("Kirana PWA — browser flows", () => {
  test("OTP login + dashboard + map picker + confirm booking", async ({
    ...
    const otpReqPromise = page.waitForResponse(
      (res) =>
        res.url().includes("/api/kirana/otp/request") &&
    ...
    // Tap map at its center to drop a pin. Leaflet container is ~full screen
    // in the modal; click its middle.
    ...
    // Picker should close → back to form, pickup button now shows the address
    ...
    // Wait up to 15s for Nominatim (rate-limited to 1 req/sec) + debounce
    ...
    const estReqPromise = page.waitForResponse(
      (res) =>
        res.url().includes("/api/kirana/bookings") &&
    ...
    // Redirects to /k/bookings
    await page.waitForURL("/k/bookings", { timeout: 10_000 });
    // At least one booking with this phone must be visible. Use .first()
    // because test re-runs accumulate bookings with this shared rider phone.
