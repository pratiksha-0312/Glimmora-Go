...
// they verify that pre-auth surfaces (PWA shell, partner signup/login) are
...
test("PWA shell files public (no auth)", async ({ browser }) => {
  ...
  expect(sw.headers()["service-worker-allowed"]).toBe("/p/");
...
test("partner login page renders + manifest linked", async ({ page }) => {
  await page.goto("/p/login");
  await expect(
    page.getByRole("heading", { name: /Partner Sign In/i })
  ).toBeVisible();
